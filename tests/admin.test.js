...
const { writeArtwork, listArtwork } = require('../src/lib/artwork-store');
const app = require('../src/app');

const ADMIN_EMAIL = 'arvestal@gmail.com,ljvestal@gmail.com';
const LOGAN_EMAIL = 'ljvestal@gmail.com';
const JWT_SECRET = 'test-jwt-secret';

function adminCookies(email = 'arvestal@gmail.com') {
  return [`admin_token=${createAdminToken(email, JWT_SECRET)}`];
}
...
describe('GET /admin/auth/google/callback', () => {
  ...
  it('logs in and sets an admin_token cookie for the first email in ADMIN_EMAIL', async () => {
    process.env.ADMIN_EMAIL = ADMIN_EMAIL;
    process.env.ADMIN_JWT_SECRET = JWT_SECRET;
    mockGetToken.mockResolvedValue({ tokens: { id_token: 'fake-id-token' } });
    mockVerifyIdToken.mockResolvedValue({ getPayload: () => ({ email: 'arvestal@gmail.com' }) });
  ...
  it('logs in a second admin email from the same comma-delimited ADMIN_EMAIL list', async () => {
    process.env.ADMIN_EMAIL = ADMIN_EMAIL;
    process.env.ADMIN_JWT_SECRET = JWT_SECRET;
    mockGetToken.mockResolvedValue({ tokens: { id_token: 'fake-id-token' } });
    mockVerifyIdToken.mockResolvedValue({ getPayload: () => ({ email: LOGAN_EMAIL }) });
  ...
  it('denies access when the verified email is not in ADMIN_EMAIL', async () => {
    process.env.ADMIN_EMAIL = ADMIN_EMAIL;
    mockGetToken.mockResolvedValue({ tokens: { id_token: 'fake-id-token' } });
...
describe('GET /admin', () => {
  ...
  it('renders the dashboard when the admin_token cookie is valid', async () => {
    process.env.ADMIN_EMAIL = ADMIN_EMAIL;
    process.env.ADMIN_JWT_SECRET = JWT_SECRET;
    const token = createAdminToken('arvestal@gmail.com', JWT_SECRET);

    const res = await request(app).get('/admin').set('Cookie', `admin_token=${token}`);
    expect(res.status).toBe(200);
    expect(res.text).toContain('arvestal@gmail.com');
  });

  it('renders the dashboard for the second admin email too', async () => {
    process.env.ADMIN_EMAIL = ADMIN_EMAIL;
    process.env.ADMIN_JWT_SECRET = JWT_SECRET;
    const token = createAdminToken(LOGAN_EMAIL, JWT_SECRET);

    const res = await request(app).get('/admin').set('Cookie', `admin_token=${token}`);
    expect(res.status).toBe(200);
    expect(res.text).toContain(LOGAN_EMAIL);
  });
...
describe('admin artwork management', () => {
  const originalDataDir = process.env.ARTWORK_DATA_DIR;
  let dataDir;

  beforeEach(() => {
    process.env.ADMIN_EMAIL = ADMIN_EMAIL;
    process.env.ADMIN_JWT_SECRET = JWT_SECRET;
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'admin-artwork-'));
    process.env.ARTWORK_DATA_DIR = dataDir;
  ...
  afterAll(() => {
    if (originalDataDir === undefined) delete process.env.ARTWORK_DATA_DIR;
    else process.env.ARTWORK_DATA_DIR = originalDataDir;
  });

  it('lists existing artwork on the dashboard', async () => {
    const res = await request(app).get('/admin').set('Cookie', adminCookies());
    expect(res.status).toBe(200);
    expect(res.text).toContain('/img/art/thumb/art-001.webp');
  ...
  it('redirects with an error when no file is uploaded', async () => {
    const res = await request(app).post('/admin/artwork').set('Cookie', adminCookies());
    expect(res.status).toBe(302);
    expect(res.headers.location).toContain('uploadError=');
    expect(listArtwork(dataDir)).toHaveLength(1);
  });
  ...
  it('deletes a piece', async () => {
    const res = await request(app)
      .post('/admin/artwork/art-001/delete')
      .set('Cookie', adminCookies());

    expect(res.status).toBe(302);
    expect(res.headers.location).toBe('/admin');
    expect(listArtwork(dataDir)).toHaveLength(0);
  });
});
